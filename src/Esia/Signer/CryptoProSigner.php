 ...
 * (the `\CPStore` / `\CPHashedData` / `\CPRawSignature` COM-like classes).
 *
 * ESIA expects the *raw* GOST signature value, byte-reversed and then
 * base64url-encoded (official methodology
 * "Методические рекомендации по интеграции с ЕСИА",
 * @see https://digital.gov.ru/ru/documents/6186/) — NOT a PKCS#7/CAdES
 * container. The implementation follows the approach proven in production by
 ...
 *
 * @see https://docs.cryptopro.ru/cades/reference/cadescom/cadescom_interface/irawsignaturesignhash
 */
final class CryptoProSigner implements SignerInterface
{
    ...
    public function sign(string $message): string
    {
        ...
        return $this->urlSafe(base64_encode(strrev(hex2bin($signature))));
    }
}
