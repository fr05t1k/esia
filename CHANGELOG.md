...
Исправление ГОСТ-подписи для КриптоПро. Оба CryptoPro-сигнера раньше
формировали контейнер **PKCS#7/CAdES**, который ЕСИА при ГОСТ-аутентификации
**не принимает**, — то есть ГОСТ-подпись через КриптоПро фактически не
работала. Теперь формируется «сырое» значение подписи по методическим
рекомендациям ЕСИА. Поскольку рабочего поведения не было, версия поднята как
minor; при этом у `CliCryptoProSigner` **изменилась сигнатура конструктора** —
если вы уже подключали его, см. миграцию ниже.

### Fixed

- `CliCryptoProSigner` переписан по методическим рекомендациям ЕСИА и
  проверенной в проде реализации @ilimurzin: вместо `cryptcp -signf`
  (который отдаёт контейнер PKCS#7) теперь используется `csptest` для получения
  «сырого» значения ГОСТ-подписи, которое разворачивается побайтово (`strrev`)
  и кодируется в base64 url-safe — как того и требует ЕСИА (см.
  [методические рекомендации](https://digital.gov.ru/ru/documents/6186/)).
  Проверено на живой установке КриптоПро CSP 5.0: подпись длиной 64 байта
  (ГОСТ Р 34.10-2012-256) формируется корректно.
  (следствие #97, спасибо @ilimurzin)
- `CryptoProSigner` (PHP-расширение КриптоПро) переведён с CAdES/PKCS#7
  (`CPSignedData->SignCades`) на «сырую» ГОСТ-подпись: хэш считается через
  `CPHashedData`, подписывается `CPRawSignature->SignHash`, результат
  разворачивается побайтово (`strrev(hex2bin(...))`) и кодируется в base64
  url-safe. Сигнатура конструктора не изменилась.
  (следствие #97, спасибо @ilimurzin)

### Changed

- **Изменена сигнатура конструктора `CliCryptoProSigner`** (потому что сменился
  и инструмент, и способ адресации ключа):
  ...
  пароль контейнера передаётся флагом `csptest -password`.
...
[Unreleased]: https://github.com/fr05t1k/esia/compare/3.1.0...HEAD
[3.1.0]: https://github.com/fr05t1k/esia/compare/3.0.1...3.1.0
