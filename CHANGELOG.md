...
- **BREAKING.** `CliCryptoProSigner` переписан по методическим рекомендациям
  ЕСИА и проверенной в проде реализации @ilimurzin: вместо `cryptcp -signf`
  ...
  PKCS#7-подпись ЕСИА при ГОСТ-аутентификации не принимает, поэтому прежний
  вариант был некорректен.
...
- **BREAKING** (по формату подписи, сигнатура конструктора прежняя).
  ...
  url-safe — как требует ЕСИА. PKCS#7 при ГОСТ-аутентификации некорректен.
  (следствие #97, спасибо @ilimurzin)
